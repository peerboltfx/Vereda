...
export default function Plantable(props){
    ...
    const {quiz, flash}=usePage().props;
    const longEnUsFormatter=new Intl.DateTimeFormat('en-GB',{
       ...
       const count = Math.ceil(quiz.length / PER_PAGE);
       const _DATA = usePagination(quiz, PER_PAGE);
                                ...
                                <tbody>
                                    {_DATA.currentData().map((data, index)=>{
                                        return(<tr key={index}>
                                            ...
                                            <td className="flex">
                                            <Link className="ml-4" href={route("viewQUestions",data.id)}><PencilSquare /></Link>
                                             </td>
                                        </tr>)
                                    })}
                                </tbody>
